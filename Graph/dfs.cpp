...
void dfs(vertex *graph, int *distance, int *label, int start)
{
	node *temp = graph[start-1].next;
	printf("%d->", start);
	while(temp != NULL)
	{
		if(distance[temp->vertexkey - 1] == 999)
		{
			label[temp->vertexkey-1] = label[start-1];
			distance[temp->vertexkey-1] = distance[start-1] + 1;
			dfs(graph, distance, label, temp->vertexkey);
		}
		temp = temp->next;
	}
}
...
int main()
{
	...
	for(i=1; i<=V; i++)
	{
		if(distance[i-1] == 999)
		{
			label[i-1] = i;
			distance[i-1] = 0;
			printf("DFS traversal of graph from vertex %d: \n", i);
			dfs(graph, distance, label, i);
			printf("\n");
		}
	}
